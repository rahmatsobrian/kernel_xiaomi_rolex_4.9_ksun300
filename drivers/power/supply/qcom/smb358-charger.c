...
#define SMB358_FAST_CHG_MIN_MA		1000
...
static int smb358_get_prop_batt_capacity(struct smb358_charger *chip)
{
	...
	dev_dbg(chip->dev, "Couldn't get bms_psy, return default capacity\n");
...
static int smb358_batt_property_is_writeable(struct power_supply *psy,
...
static const struct power_supply_desc smb358_USB_desc = {
    .name = "usb",
	.type = POWER_SUPPLY_TYPE_USB,
	.get_property = smb358_usb_get_property,
	.set_property = smb358_usb_set_property,
	.properties = smb358_usb_properties,
	.num_properties = ARRAY_SIZE(smb358_usb_properties),
	.property_is_writeable = smb358_usb_is_writeable,
};

static const struct power_supply_desc smb358_battery_desc = {
    .name           = "battery",
    .type           = POWER_SUPPLY_TYPE_BATTERY,
    .properties     = smb358_battery_properties,
    .num_properties = ARRAY_SIZE(smb358_battery_properties),
    .get_property   = smb358_battery_get_property,
    .set_property   = smb358_battery_set_property,
    .property_is_writeable = smb358_batt_property_is_writeable,
    .external_power_changed = smb358_external_power_changed,
};
				...
				const struct i2c_device_id *id)
{
	...
	chip->extcon = devm_extcon_dev_allocate(chip->dev, smb358_extcon_cable);
	if (IS_ERR(chip->extcon)) {
		rc = PTR_ERR(chip->extcon);
		dev_err(chip->dev,
			"failed to allocate extcon device rc = %d\n",
			rc);
		return rc;
	}
	...
	if (IS_ERR(chip->usb_psy)) {
		...
		dev_err(chip->dev,
			"Unable to register usb_psy rc = %d\n",
			rc);
		return rc;
	}
	...
	if (rc) {
		dev_err(chip->dev,
			"failed to register extcon device rc = %d\n",
			rc);
		return rc;
	}
	...
	if (IS_ERR(chip->batt_psy)) {
		...
		dev_err(chip->dev,
			"Couldn't register batt psy rc=%d\n", rc);
		return rc;
	}
